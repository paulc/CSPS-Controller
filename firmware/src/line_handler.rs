...
fn dps_read(i2c: &mut I2c<'static, I2C1, Blocking>, cmd: u8) -> Result<u16, CmdError> {
    ...
    let mut buf = [0u8; 3];
    i2c.blocking_write_read(DPS_ADDR, &[cmd, cs], &mut buf)
                            ...
                            for (measure, cmd, scale, unit) in DPS_CMDS {
                                match dps_read(&mut i2c, *cmd) {
                                    Ok(v) => {
                                        ...
                                        serial_write(b"]");
                                        serial_write(b"\r\n");
                                        // Give TX_PIPE a chance to drain - not actually needed
                                        // here as output just fits in buffer. Adds 86 bytes .text
                                        // and 88 bytes .bss for yield point.
